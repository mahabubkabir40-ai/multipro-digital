          ...
          {filteredResults.map((result) => (
            ...
            >
              <div className="relative aspect-[4/3] w-full overflow-hidden">
                ...
                {/* Info Hierarchy */}
                <div className="absolute bottom-6 left-6 right-6 z-30 transition-transform duration-500 group-hover:-translate-y-44 group-active:-translate-y-44">
                  ...
                  <h3 className="text-white font-serif font-bold text-2xl mt-1 drop-shadow-md leading-snug">
                    {result.name}
                  </h3>
                </div>

                {/* Hover Reveal Content (Metrics & CTA) */}
                <div className="absolute bottom-0 left-0 right-0 p-6 z-30 translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 group-active:translate-y-0 group-active:opacity-100 transition-all duration-500 delay-75 flex flex-col justify-end h-full">
                  
                  <div className="mb-6 border-l-[3px] border-brand-lime pl-4">
                    <div className="flex items-end gap-2 text-brand-lime">
                      <span className="text-5xl font-black leading-none tracking-tighter">{result.metric}</span>
                    </div>
                    <p className="text-white font-bold text-base tracking-wide mt-2">{result.metricLabel}</p>
                    <p className="text-white/60 text-sm font-sans mt-1">{result.timeframe}</p>
                  </div>

                  <Link 
                    href="/contact#audit-form"
                    className="relative group overflow-hidden w-full py-3.5 rounded-xl bg-brand-lime text-[#1A365D] font-black text-[10px] min-[375px]:text-xs sm:text-sm tracking-wide transition-all duration-300 active:duration-75 transform hover:scale-[1.05] active:scale-90 active:bg-white shadow-[0_4px_14px_rgba(154,251,22,0.3)] hover:shadow-[0_0_30px_rgba(154,251,22,0.6)] flex items-center justify-center gap-2 whitespace-nowrap select-none touch-manipulation"
                    style={{ WebkitTapHighlightColor: 'transparent' }}
                  >
